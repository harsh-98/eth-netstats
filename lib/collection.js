...
Collection.prototype.updateActivePkh = function(data, callback)
{
	var node = this.getNode({ id: data.id });
	...
	this._activePkh = data.count;
	callback(this._activePkh);
}
Collection.prototype.updateSuperBlock = function(data, callback)
{
	var node = this.getNode({ id: data.id });

	if (!node.canUpdate() || data.super.index < this._superBlock.index ) {
		callback(null);
		return;
	}


	this._superBlock = data.super;
	callback(data.super);
}
...
Collection.prototype.all = function()
{
	...
	return {nodes: this._items, active: this._activePkh, superBlock: this._superBlock};
}
