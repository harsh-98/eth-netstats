...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.data_request_txns) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
...
History.prototype._save = function(block)
{
	...
	while(this._items.length > MAX_HISTORY)
	{
		this._items.pop();
	}
}
...
History.prototype.prevMaxBlock = function(number)
{
	...
	if(index < 0 || number -1 != this._items[index].height)
		return false;
...
History.prototype.getRADCount = function()
{
	var request = _( this._items )
		.sortByOrder( 'height', false )
		...
		.map(function (item)
		{
			return item.block.data_request_txns.length;
		})
		.value();

	console.log("RADrequest", request)
	return request;
}
...
History.prototype.getCharts = function()
{
	...
	{	
		// console.log(_( this._items ).sortByOrder( 'height', false ).slice(0, 2).reverse().value())
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
				return {
					...
					RADCount: item.block.data_request_txns.length,
					transactions: item.block.transactions.length,
					txfees: item.block.txfees,
